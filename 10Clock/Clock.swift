...
//        print("start = \(headAngle / CGFloat(Double.pi)), end = \(tailAngle / CGFloat(Double.pi))")
    ...
    func tlabel(_ str:String, color:UIColor? = nil) -> CATextLayer{
        let f = UIFont.preferredFont(forTextStyle: UIFont.TextStyle.caption1)
    ...
    func updateWatchFaceTitle(){
        let f = UIFont.preferredFont(forTextStyle: UIFont.TextStyle.title2)
        let cgFont = CTFontCreateWithName(f.fontName as CFString, f.pointSize/2,nil)
//        let titleTextLayer = CATextLayer()
        titleTextLayer.bounds.size = CGSize( width: titleTextInset.size.width, height: 80)
